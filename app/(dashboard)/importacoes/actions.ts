...
/** Feedback-friendly variant of importAction, for ActionForm/useActionState. */
export async function importFormAction(
  state: FormActionState<ImportBatchRow>,
  formData: FormData,
): Promise<FormActionState<ImportBatchRow>> {
  return runFormAction("imports", "create", paths, state, async (actorId) => {
    const entity = String(formData.get("entity") ?? "")
    const dryRun = String(formData.get("dryRun") ?? "true") !== "false"
    if (!isImportEntity(entity)) throw new KnownFormError("Entidade de importação inválida.")

    const file = formData.get("file")
    if (!(file instanceof File) || file.size === 0) throw new KnownFormError("Selecione um arquivo CSV.")
    if (file.size > IMPORT_LIMITS.maxBytes) {
      throw new KnownFormError(`Arquivo excede o tamanho máximo de ${IMPORT_LIMITS.maxBytes} bytes.`)
    }
